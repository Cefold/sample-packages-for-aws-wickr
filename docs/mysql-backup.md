...
1. Put server offline, by deploying Ent 6.58.1 KOTS config change (“Database Upgrade Confirmation” )
2. Backup DB
3. Migrate to MySQL 8, by deploying Ent 6.62.x
    1. Check for update if preflight check failed
    2. Ingress/server will be back online alone with the upgrade
4. Rollback and restore (only if needed)

It is crucial to put server offline to prevent database being updated during the migration. The server can be brought back once the migration or rollback is complete.
...
* Get DB Password
    ...
    `export MYSQL_ROOT_PASSWORD=$(kubectl get secrets mysql -n wickr -o jsonpath='{.data.mysql-root-password}' | base64 -d)`
* Export DB Dump File
